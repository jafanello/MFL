...
def adjust_data_qi(xydata, n_shots=1, omegabounds=[0, 15], extract_method="maj_voting", loss_level=1):
    """
    ...
    elif extract_method == "sample_binomial":
        round_ydata = np.random.binomial(n_shots, p=Ramsey_ydata)
        adjdata = np.column_stack([n_shots * round_ydata, Ramsey_xdata, n_shots * np.ones_like(Ramsey_ydata)])
    elif extract_method == "sample_unbalanced":
        round_ydata = np.random.binomial(n_shots, p=Ramsey_ydata)
        for i in range(len(round_ydata)):
            if round_ydata[i] == 1:
                round_ydata[i] = np.random.binomial(1, p=loss_level)
        adjdata = np.column_stack([n_shots * round_ydata, Ramsey_xdata, n_shots * np.ones_like(Ramsey_ydata)])
    else:
        raise Exception("Unknown method for extracting data")
...
def PoisDampedOscill(t, Omega, invT2, shots):
    y = 1 - np.exp(-t * invT2) * np.cos(Omega * t / 2) ** 2 - 0.5 * (1 - np.exp(-t * invT2))
    exp_counts = np.random.poisson(shots * y)
    return (exp_counts / shots)
...
def scaling_fit(scale_fit, acctimes, medcov, errcov, p0=None, skip_start=2, skip_end=-1):
    if errcov is not None:
        ...
        sigma = None
    popt, pcov = curve_fit(scale_function, acctimes[skip_start:skip_end], medcov[skip_start:skip_end], p0=p0,
                           sigma=sigma)
...
def time_analysis(npoutput, out_index, convert_factor=1., plot_color=None, plot_title="",
                  enable_save=False, todir=None, tofile=None, return_hist=False):
    all_pgh_times = []
    ...
    myhist = plt.hist(np.array(all_pgh_times), log=True, bins=10 ** np.linspace(np.log10(MIN), np.log10(MAX), 50),
    ...
    if return_hist:
        return [np.array(all_pgh_times) * convert_factor, medtimes, myhist]
    else:
...
class DataPrecModel():
    ...
    def est_prec(self, data, n_shots=1, n_experiments=50, resample_a=None, resample_thresh=0.5,
                 extract_method="maj_voting", loss_level=1, heuristic="PGH",
                 verbose=False, TBC=False):
        """
        ...
        if TBC:
            ...
            if self.wDeco is "ToLearn":
                self.prior = qi.ProductDistribution(qi.UniformDistribution([self.freq_min, self.freq_max]),
                                                    qi.NormalDistribution(self.mean_T, self.var_T))
        ...
        if data is None:
            ...
            mydata = adjust_data_qi(data, n_shots=n_shots, omegabounds=[self.freq_min, self.freq_max],
                                    extract_method=extract_method, loss_level=loss_level)
        ...
        for idx_experiment in range(n_experiments):
            ...
            if heuristic is None:
                try:
                    ...
                    warnings.warn("No additional times available, learning from last instance, with PGH=None")
                    experiment[0][0] = data[0][-1]
    ...
    def est_stepwise(self, data, chosen_ones, n_shots=1, n_experiments=50, resample_a=None, resample_thresh=0.5,
                     wait_resample=10, wait_cata=50, brake_sigma=10 ** 0, verbose=False,
...
class NoisyDataPrecModel():
    ...
    def est_prec(self, data, n_shots=1, n_experiments=50, resample_a=None, resample_thresh=0.5,
                 extract_method="maj_voting", loss_level=1, heuristic="PGH",
                 verbose=False, TBC=False, use_heuristic="PGH", heuristic_ratio=1., heuristic_root=1):
        """
        Class for (decohered) Ramsey fringe learning
